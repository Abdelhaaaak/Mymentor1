    ...
    <aside class="w-1/3 border-r overflow-auto">
      <ul>
        @forelse($contacts as $contact)
          <li>
            <a
              href="{{ route('messages.index', [$queryKey => $contact->id]) }}"
              class="flex items-center px-4 py-3 hover:bg-gray-100 {{ optional($selectedUser)->id === $contact->id ? 'bg-indigo-50 font-semibold' : '' }}"
            >
              <img
                src="{{ $contact->profile_image_url ?? 'https://via.placeholder.com/40' }}"
      ...
      <div class="flex-1 p-4 overflow-auto space-y-4">
        @if($selectedUser)
          @forelse($messages as $msg)
            <div
              class="max-w-xs px-4 py-2 rounded-lg {{ $msg->sender_id === auth()->id() ? 'bg-indigo-100 self-end' : 'bg-gray-100 self-start' }}"
            >
              {{ $msg->content }}
